<?php
//セキュリティ対策のため、データベース名、ユーザー名、パスワードは非表示にしています
$dsn = '';
$user = '';
$db_password = '';
...
if (isset($_POST['name']) && isset($_POST['comment']) && isset($_POST['toko_pass'])) {
    ...
    if (!empty($toko_check)) {   //投稿が新規か編集かチェック（$toko_checkが空でなかったら、編集対象の投稿内容だと判断）
        ...
        foreach ($results as $row) {
            if ($row['toko_pass'] == $toko_pass) {    //パスワードが一致したら投稿内容を編集
    ...
    } else {    //$toko_checkが空だったら、新規の投稿内容だと判断
...
} elseif (isset($_POST['sakujo']) && isset($_POST['sakujo_pass'])) {    //削除部分の処理
    ...
    if (!empty($sakujo_id) && !empty($sakujo_pass)) {
	    ...
	    foreach ($results as $row){
            ...
            if ($row['id'] == $sakujo_id) {    //投稿番号が一致したら

                if ($row['toko_pass'] == $sakujo_pass) {      //かつパスワードが一致したら、投稿内容を削除
...
} elseif (isset($_POST['henshu']) && isset($_POST['henshu_pass'])) {    //編集部分の処理（編集する投稿の、投稿主と投稿内容を取得）
    ...
    if (!empty($henshu_id) && !empty($henshu_pass)) {
        ...
        foreach ($results as $row) {
            ...
            if ($row['id'] == $henshu_id) {    //投稿番号が一致したら
                ...
                if ($row['toko_pass'] == $henshu_pass) {    //かつパスワードが一致したら投稿主の名前と投稿内容を取得
